# fem.py
# Free Energy Mixer (FEM): single-pass value mixing, backend auto-detection, fully-parallel fallbacks
# Stable LSE: -max trick (no clamp after shift) + stable_log_restore (exact, no leakage)
...
class FreeEnergyMixerConfig:
    ...
    n_embd: int
    n_head: int
    fem_dim: Optional[int] = None  # defaults to n_embd if None
    
    # When fem_dim is None, compute fem_dim = round(n_embd * fem_ratio) to nearest multiple of n_head.
    fem_ratio: float = 0.5
    
    # Per-token dimension of each of Q *or* K (i.e., out_features of W_q / W_k).
    # If None, it will be computed from p_t_to_fem_ratio and fem_dim:
    #   qk_dim = 0.5 * p_t_to_fem_ratio * fem_dim
    # Then rounded to a multiple of n_head.
    qk_dim: Optional[int] = None

    # Total parameterization size p_t (Q+K) relative to fem_dim.
    # Default 4.0: fem_dim = n_embd//2 and |Q|+|K| = 2*n_embd.
    p_t_to_fem_ratio: float = 4.0

    # Priors: "softmax" | "linear" | "gla" | "rnn_softmax" | "mamba"
    prior_type: str = "softmax"

    # Softmax backend:
    #   "auto"  -> prefer SDPA (supports v_dim != qk_dim). Only use flash-attn if explicit and compatible.
    #   "flash" -> try flash-attn (requires v_dim == qk_dim), else SDPA.
    #   "naive" -> naive matmul softmax (debug) — we still use SDPA for brevity.
    ...
    # FEM knobs
    beta_max_init: float = 1.0
    value_logexp_cap: float = 40.0   # cap for |β·v| via pre-clip on v (no clamp after subtracting max)
    eps: float = 1e-6

    # Feature flags
    use_temperature: bool = True     # learn β and enable exp branch
    use_lse: bool = True             # enable LSE branch when True (β=1 if use_temperature=False)
    use_outer_gate: bool = True      # multiplicative outer gate
    use_rope: bool = True            # apply RoPE to Q/K
...
def _expand_per_channel(vec: torch.Tensor, like: torch.Tensor) -> torch.Tensor:
    """vec: (E,) -> (1,1,H,d_v) to match 'like' of shape (B,T,H,d_v)."""
    _, _, H, d = like.shape
...
def _round_channels(
    ...
    Round x to a value that:
      ...
      2) is divisible by `n_head * per_head_unit`
         (so per-head dim = x / n_head is divisible by `per_head_unit`)
    Use per_head_unit=2  -> per-head dim even
        per_head_unit=4  -> per-head dim % 4 == 0 

    mode="nearest": choose the closest valid value (ties → larger one)
         "up":     always round up to the next valid
         "down":   always round down to the previous valid
    """
    ...
    if base <= 0:
        # Degenerate; fall back to enforce only head-split constraint
        base = base_req
    ...
    if mode == "up":
        ...
        q = max(q, 1)
        cand = q * base
        # If exactly halfway between two multiples, prefer the larger one
        low = (q - 1) * base if q > 1 else base
        high = (q + 1) * base
        if abs(cand - x) < abs(low - x) and abs(cand - x) <= abs(high - x):
            return cand
        # pick closer between low/high; ties -> high
...
# -----------------------------------------------------------------------------
# Time-Decay Condition (fully-parallel)
# -----------------------------------------------------------------------------
class TimeDecayConditionLayer(nn.Module):
    ...
    Closed-form time-decay accumulation (parallel):
      ...
      out = D * cumsum( (x_f / D) )
    We multiply a normalized shortcut, then project.
    """
    ...
    def forward(self, x: torch.Tensor, gated_output: bool = True) -> torch.Tensor:
        ...
        c = a.cumsum(dim=1)
        D = torch.exp(c)
        out = (xf / (D + 1e-12)).cumsum(dim=1) * D
        out = sc * self.ln(out)
        out = out @ self.out_proj
        return x * out if gated_output else out
...
# -----------------------------------------------------------------------------
# Stable exp branch utilities (NO clamp after subtracting max)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Softmax prior (single-pass over concatenated values)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Linear prior (single-pass, fully-parallel)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Gated Linear Attention (single-pass, fully-parallel)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# AFT-like Linear RNN (expand_qk; fully-parallel)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Mamba SSM (efficient with selective_scan_fn) + fallback EMA
# -----------------------------------------------------------------------------
...
class FreeEnergyMixer(nn.Module):
    def __init__(self, cfg: FreeEnergyMixerConfig):
        ...
        if cfg.fem_dim is None:
            # target by ratio; ensure per-head dim divisible by 4 (i.e., E/head % 4 == 0),
            # so that E/head/2 
            ...
            E = _round_channels(
                ...
                n_head=H,
                per_head_unit=4,          # => (E / H) % 4 == 0  → RoPE-safe & nicer packing
                mode="nearest",
            )
        else:
            E = _round_channels(
        ...
        # -------- qk_dim (per-token total dim for each of Q or K) --------
        if cfg.prior_type.lower() in {"softmax", "linear", "gla", "rnn_softmax"}:
            if getattr(cfg, "qk_dim", None) is not None:
                qk_total = _round_channels(
                    x=int(cfg.qk_dim),
                    multiple=getattr(cfg, "align_multiple", 1),
                    n_head=H,
                    per_head_unit=2,      # => (qk_total / H)
                    mode="nearest",
                )
            else:
                # qk_total ≈ 0.5 * p_t_to_fem_ratio * E
        ...
        # RoPE uses d_k (may differ from D/H now)
    ...
    def forward(self, x: torch.Tensor, attention_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if attention_mask is not None:
            m = attention_mask.to(x.dtype).unsqueeze(-1).unsqueeze(-1)
        ...
        if self.tdc is not None:
            ...
            if (self.cfg.use_temperature or self.cfg.use_lse):
                s_lam = tdc_out[:, :, idx:idx + self.E]; idx += self.E
            else:
                s_lam = None
            if self.cfg.use_outer_gate:
                s_gate = tdc_out[:, :, idx:idx + self.E]; idx += self.E
            else:
                s_gate = None
            if self.prior_type == "gla":
                _ = tdc_out[:, :, idx:idx + self.H]
        else:
            s_lam = s_gate = None
        ...
        if use_branch:
            if self.cfg.use_temperature:
                ...
                beta_vec = _softplus_pos(self._beta_param + 0.8, self.eps)           # (E,)
        ...
        if pt == "softmax":
            if use_branch:
                ...
                fmax = stable_log_restore(z_exp_scaled, v_max_shift, detach_M=True)
            else:
